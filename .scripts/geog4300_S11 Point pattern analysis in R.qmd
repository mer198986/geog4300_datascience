...
dollars<-read_csv("~/Downloads/dollars.csv") %>%
  st_as_sf(coords=c("x","y"),crs=4326)

atl_pumas<-st_read("~/Downloads/atl_pumas.gpkg")
```
...
dollars_puma_count_wide_v2<-dollars_puma_count %>%
  pivot_wider(names_from=dollar_type,
              values_from=n,
              values_fill=0) %>% #values_fill replaces any NAs with 0
  mutate(total_stores=`Dollar General`+`Family Dollar`+`Dollar Tree`,
...
atl_pumas_join_v2<-atl_pumas %>%
  left_join(dollars_puma_count_wide_v2)

tm1<-tm_shape(atl_pumas_join_v2)+ tm_polygons("fd_pct")
tm2<-tm_shape(atl_pumas_join_v2)+ tm_polygons("dg_pct")
tm3<-tm_shape(atl_pumas_join_v2)+ tm_polygons("dt_pct")
...
dollars_puma%>%
  st_join(grid2 %>% st_transform(4326), #st_transform changes the projection to the listed EPSG number
          join=st_within) %>%
  filter(is.na(GEOID)==FALSE)

#Plot stores by tract
tm_shape(dollars_puma) + 
  tm_shape(dollars_puma_df)+tm_polygons(count(dollar_type))
